...
- The hosted **`get_context()` memory layer** — it assembles the ~2,000-token block behind the [LongMemEval 76-vs-42 result](token-savings/) from selected verbatim turns plus, where available, resolved bi-temporal facts (contradiction-aware). That *selection* runs in the cloud, not here; the open engine in this repo is the retrieval core it is built on (`search`, `get_related`, the entity graph).
